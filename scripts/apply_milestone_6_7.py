...
def patch_api() -> None:
    ...
    import_line = "from .api_v26 import router as milestone_26_router\n"
    include_line = "app.include_router(milestone_26_router)\n"

    if import_line not in text:
        marker = "from .api_v25 import router as milestone_25_router\n"
        if marker not in text:
            raise SystemExit("Milestone 25 router import was not found")
        text = text.replace(marker, marker + import_line, 1)

    if include_line not in text:
        marker = "app.include_router(milestone_25_router)\n"
        if marker not in text:
            raise SystemExit("Milestone 25 router registration was not found")
...
def patch_database() -> None:
    ...
    if "api_key_telemetry_models" in text:
        return
    ...
    names.add("api_key_telemetry_models")
...
def main() -> None:
    patch_api()
    patch_database()
    print("Milestone 6.7 API key telemetry added")
